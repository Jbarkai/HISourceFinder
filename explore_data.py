...
#     cube_data = fits.getdata("./data/training/Input/noisefree_" + mask.split("_")[-1])
#     bbs = [gal.bbox for gal in some_props]
#     del some_props
#     gc.collect()
#     tot_fluxes = [np.sum(cube_data[bbs[i][0]:bbs[i][3], bbs[i][1]:bbs[i][4], bbs[i][2]:bbs[i][5]])
...
galdim = []
pixel_percents = []
masks = [i for i in listdir("./data/training/Input")if ".fits" in i]
...
plt.show()
fig1.savefig('./plots/tot_flux.png')
...
plt.show()
fig1.savefig('./plots/peak_flux.png')
...
plt.show()
fig1.savefig('./plots/eccentricity.png')
...
plt.ylabel("Flatness")
plt.show()
fig1.savefig('./plots/flatness.png')
...
plt.show()
fig1.savefig('./plots/pixel_percents.png')
