...
def create_fake_cube(i, no_cubes, noise_file, gal_dir, out_dir):
    ...
    success = [add_to_cube(
        j, no_gals, g, noise_cube, noise_data, empty_cube
...
def main(no_cubes, mos_dir, gal_dir, out_dir):
    ...
    Returns:
        The return value. True for success, False otherwise.
    """
    cubes = sample([k for k in listdir(mos_dir) if ".fits" in k], no_cubes)
    success = [create_fake_cube(k, no_cubes, f, gal_dir, out_dir) for k, f in enumerate(cubes)]
    print("Success! ", success)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        'gal_dir', type=str, nargs='?', const='default', default='data/mock_gals/smoothed',
    ...
    parser.add_argument(
        'no_cubes', type=int, nargs='?', const='default', default=100,
        help='The number of synthetic training cubes to produce')
    args = parser.parse_args()

    main(args.no_cubes, args.mos_dir, args.gal_dir, args.out_dir)
