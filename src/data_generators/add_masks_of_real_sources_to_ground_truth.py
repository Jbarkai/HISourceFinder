...
def main(args, data_dir="./data/", scale="loud"):
    ...
    # Take only real ones
    real_gals = pd.merge(new_mto[~new_mto.type.isnull() & (new_mto.type!="notsure") & (new_mto.mos_name.str.contains("1353"))], pd.merge(new_sof[~new_sof.type.isnull() & (new_sof.type!="notsure") & (new_sof.mos_name.str.contains("1353"))], new_vnet[~new_vnet.type.isnull() & (new_vnet.type!="notsure") & (new_vnet.mos_name.str.contains("1353"))], on=['mos_name', 'max_loc'], how='outer', suffixes=("_sof", "_vnet")), on=['mos_name', 'max_loc'], how='outer', suffixes=("_mto", ""))
    ...
    mto_sources = mto_sources[mto_sources.columns[~mto_sources.columns.str.contains("_sof") & ~mto_sources.columns.str.contains("_vnet")]]
    mto_sof_sources = real_gals[~real_gals.type_sof.isnull() & ~real_gals.type.isnull()]
    # merge all the sources together
    # Add SoFiA sources:
    for mos_name in [i for i in new_sof.mos_name.unique() if "1353" in i]:
        print(mos_name)
        arr_shape = (652, 1800, 2400)
        mask_cube = np.zeros(arr_shape)
        # mask_cube = fits.getdata("./data/training/Target/mask_%s.fits"%mos_name)
        mto_data = fits.getdata(data_dir + "mto_output/mtocubeout_" + scale + "_" + mos_name+  ".fits")
        vnet_data = fits.getdata(data_dir + "vnet_output/vnet_cubeout_" + scale + "_" + mos_name+  ".fits")
        sofia_data = fits.getdata(data_dir + "sofia_output/sofia_" + scale + "_" + mos_name+  "_mask.fits")
        ...
        fits.writeto("data/training/TargetReal/mask_%s.fits"%mos_name, mask_cube, overwrite=True)
    return
