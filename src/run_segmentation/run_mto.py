...
# def mto_eval(window, mto_dir, param_file, empty_arr, index, sigma):
def mto_eval(f_in, mto_dir, param_file, empty_arr, sigma):
    # cube_files, x, y, z = window
    # subcube = fits.getdata(cube_files[0])[x[0]:x[1], y[0]:y[1], z[0]:z[1]]
    subcube = fits.getdata(f_in)
    # Save as fits file
    ...
    fout.close()
    # Smooth and clip
    smoothed = ndi.gaussian_filter(subcube, sigma=sigma)
    ...
    maskcube_file = mto_dir + "/masksubcube_" + f_in.split("/")[-1]
    output_file = "data/mto_output/outputcube_" + f_in.split("/")[-1]
...
def make_cube(f_in, mto_dir, param_file, sigma):
...
#         mto_eval(window, mto_dir, param_file, empty_arr, index, sigma)
    mto_eval(f_in, mto_dir, param_file, empty_arr, sigma)
...
def main(mto_dir, param_file, input_dir):
    ...
    time_taken = {}
    # Load test data
    cubes = sorted([input_dir + "/" + x for x in listdir(input_dir) if "1245mosC.fits" in x])
    sigmas = np.linspace(0.85, 1.85, 12)
    for i, f_in in enumerate(cubes):
        before = datetime.now()
        print(f_in)
        make_cube(f_in, mto_dir, param_file, sigmas[i])
        after = datetime.now()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--input_dir', type=str, nargs='?', const='default', default="data/training/InputBoth",
